...
const hasMinimumSkills = (skills, minimum = 3) =>
...
const seekerChecks = (profile = {}) => [
  ...
  },
  {
    ...
    done: hasMinimumSkills(profile.skills, 3)
  },
  {
    key: "industryField",
...
const normalizeMissing = (missingProfileFields = []) =>
  missingProfileFields
    .map((item) => {
      if (!item) return null;
      if (typeof item === "string") return { key: item, label: item };
      if (typeof item === "object" && item.key && item.label) return item;
      return null;
    })
    .filter(Boolean);
...
export const getProfileCompletionState = (user) => {
  ...
  if (typeof user.profileCompleted === "boolean") {
    return {
      ...
      missingProfileFields: normalizeMissing(user.missingProfileFields)
    };
  }
